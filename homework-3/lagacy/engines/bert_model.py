...
class BiLSTM_CRFModel(tf.keras.Model):
    def __init__(self, configs, vocab_size, num_classes, **kwargs):
        ...
        self.l_bert = TFAutoModel.from_pretrained('microsoft/deberta-v3-large')
        pooled_output = self.l_bert([input_word_ids, input_mask])
        self.model = Model(inputs=[input_word_ids, input_mask], outputs=[pooled_output])
    ...
    def call(self, X_batch, attention_mask_batch, segment_batch, inputs_length, targets, training=None):
        ...
        embedding_inputs = self.l_bert([X_batch, attention_mask_batch], training=training)[1]
