...
class Predictor:
    def __init__(self, configs, data_manager, logger, bilstm_crf_model):
        ...
        if (bilstm_crf_model == None):
            ...
            checkpoint.restore(tf.train.latest_checkpoint(configs.checkpoints_dir))  # 從文件恢復模型參數
    ...
    def evaluate_test(self, data_manager, epoch=None):
        ...
        for iteration in tqdm(range(num_test_iterations)):
            ...
            batch_pred_sequence_test, _ = crf_decode(logits_test, transition_params_test, inputs_length_test)
        ...
        predit_file = self.configs.datasets_fold + '/' + 'label_test_' + str(epoch) + '.txt'
